 ...
 * Source: src\services\WorkspaceManager.ts
 */
...
describe('WorkspaceManager (TypeScript)', () => {
    ...
    beforeEach(() => {
        ...
        sandbox.stub(Logger, 'getInstance').returns(loggerStub as any);
    ...
    describe('VSCode Integration', () => {
        ...
        it('should format document successfully', async () => {
            const executeCommandStub = vscode.commands.executeCommand;
            const mockDocument = { uri: 'test-document' };

            await workspaceManager.formatDocument(mockDocument);

            assert(executeCommandStub.calledOnce);
            assert(executeCommandStub.calledWith('editor.action.formatDocument', mockDocument));
        });

        it('should handle errors when formatting document', async () => {
            const executeCommandStub = vscode.commands.executeCommand;
            executeCommandStub.rejects(new Error('Format error'));
            const mockDocument = { uri: 'test-document' };
