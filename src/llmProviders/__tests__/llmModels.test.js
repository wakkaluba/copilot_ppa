const { LLMModelsManager } = require('../llmModels');
...
describe('LLMModelsManager', () => {
    ...
    describe('Initialization', () => {
        it('should initialize correctly', async () => {
            ...
            expect(manager._service.initialize).toHaveBeenCalled();
        });
    });
    ...
    describe('Model Management', () => {
        it('should refresh installed models', async () => {
            ...
            expect(manager._service.refreshInstalledModels).toHaveBeenCalled();
        });
        ...
        it('should get installed models', async () => {
            ...
            manager._service.getInstalledModels.mockResolvedValueOnce(mockModels);

            const result = await manager.getInstalledModels();

            expect(result).toEqual(mockModels);
            expect(manager._service.getInstalledModels).toHaveBeenCalled();
        });
        ...
        it('should get available models', async () => {
            ...
            manager._service.getAvailableModels.mockResolvedValueOnce(mockModels);

            const result = await manager.getAvailableModels();

            expect(result).toEqual(mockModels);
            expect(manager._service.getAvailableModels).toHaveBeenCalled();
        });
        ...
        it('should get all models', async () => {
            ...
            manager._service.getAllModels.mockResolvedValueOnce(mockModels);

            const result = await manager.getAllModels();

            expect(result).toEqual(mockModels);
            expect(manager._service.getAllModels).toHaveBeenCalled();
        });
        ...
        it('should get a specific model', async () => {
            ...
            manager._service.getModel.mockResolvedValueOnce(mockModel);

            const result = await manager.getModel('test-model');

            expect(result).toEqual(mockModel);
            expect(manager._service.getModel).toHaveBeenCalledWith('test-model');
        });

        it('should download a model', async () => {
            await manager.downloadModel('test-model');

            expect(manager._service.downloadModel).toHaveBeenCalledWith('test-model');
        });

        it('should delete a model', async () => {
            await manager.deleteModel('test-model');

            expect(manager._service.deleteModel).toHaveBeenCalledWith('test-model');
        });
    });
    ...
    describe('Provider Status', () => {
        it('should check Ollama status', async () => {
            ...
            manager._service.checkOllamaStatus.mockResolvedValueOnce(mockStatus);

            const result = await manager.checkOllamaStatus();

            expect(result).toEqual(mockStatus);
            expect(manager._service.checkOllamaStatus).toHaveBeenCalled();
        });

        it('should check LM Studio status', async () => {
            const mockStatus = { running: true, port: 1234 };

            manager._service.checkLMStudioStatus.mockResolvedValueOnce(mockStatus);

            const result = await manager.checkLMStudioStatus();

            expect(result).toEqual(mockStatus);
            expect(manager._service.checkLMStudioStatus).toHaveBeenCalled();
        });

        it('should start Ollama', async () => {
            await manager.startOllama();

            expect(manager._service.startOllama).toHaveBeenCalled();
        });

        it('should stop Ollama', async () => {
            await manager.stopOllama();

            expect(manager._service.stopOllama).toHaveBeenCalled();
        });
    });
    ...
    describe('Events', () => {
        it('should propagate model changed events', () => {
            ...
            manager._service.emitModelsChanged();
