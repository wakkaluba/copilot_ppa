...
export class CodeExampleService {
    ...
    constructor(context: vscode.ExtensionContext) {
        this.githubApiService = new GitHubApiService(context);
    }
    ...
    public async searchExamples(
        ...
        try {
            ...
            const githubItems = await this.githubApiService.searchCodeExamples(
                query,
                contextInfo?.language,
                contextInfo?.maxResults || 5
            );
     ...
     */
    public filterExamplesByRelevance(
        ...
        return examples
            .filter(example => {
                ...
                if (context.language && example.language) {
                    ...
                    if (exampleLang !== contextLang) {
                        ...
                        if (
                            !(exampleLang === 'typescript' && contextLang === 'javascript') &&
            ...
            .sort((a, b) => {
                ...
                for (const keyword of context.keywords) {
                    if (a.code.includes(keyword)) scoreA += 1;
                    if (b.code.includes(keyword)) scoreB += 1;
                }
