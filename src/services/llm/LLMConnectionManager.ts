...
import { ConfigurationError, LLMConnectionError } from './errors';
...
export class LLMConnectionManager extends EventEmitter implements vscode.Disposable {
    ...
    setProvider(provider: LLMProvider): boolean {
        if (!provider) {
            throw new ConfigurationError('Provider cannot be null or undefined', '', 'provider');
        }
        ...
        if (!validationResult.isValid) {
            ...
            throw new ConfigurationError(`Invalid LLM provider: ${errors}`, provider?.getName?.() || '', 'provider');
        }
    ...
    async connectToLLM(): Promise<boolean> {
        if (!this.provider) {
            throw new LLMConnectionError('NO_PROVIDER', 'No provider set - call setProvider first');
        }
        ...
        try {
            ...
            if (!isAvailable) {
                throw new LLMConnectionError('PROVIDER_UNAVAILABLE', 'Provider is not available');
            }
            ...
            this.setStatus(ConnectionStatus.Error, new LLMConnectionError('CONNECTION_FAILED', `Failed to connect: ${errorMessage}`));
    ...
    setConnectionTimeout(timeoutMs: number): void {
        if (timeoutMs < 1000) {
            throw new ConfigurationError('Timeout must be at least 1000ms (1 second)', '', 'connectionTimeout');
        }
    ...
    setMaxConnectionAttempts(attempts: number): void {
        if (attempts < 1) {
            throw new ConfigurationError('Max connection attempts must be at least 1', '', 'maxConnectionAttempts');
        }
    ...
    private async withTimeout<T>(promise: Promise<T>, timeoutMs: number, message: string): Promise<T> {
        return Promise.race([
            ...
            new Promise<T>((_, reject) => {
                setTimeout(() => {
                    reject(new LLMConnectionError('TIMEOUT', message));
                }, timeoutMs);
            })
        ]);
    }
