 ...
 */
const DEFAULT_CONFIG: Required<Omit<IProviderConfig, 'apiKey' | 'apiEndpoint' | 'customHeaders' | 'proxySettings'>> = {
    timeout: 30000,
    maxRetries: 3,
    batchSize: 10,
    modelName: 'default',
...
export class ProviderConfigManager extends EventEmitter {
    ...
    private defaults: Map<string, Partial<IProviderConfig>> = new Map();
    private configCache: Map<string, IProviderConfig> = new Map();
    ...
    private async loadPersistedConfigs(): Promise<void> {
        try {
            ...
            for (const providerId of Object.keys(config)) {
                const providerConfig = config.get<IProviderConfig>(providerId);
                if (providerConfig) {
    ...
    private getCachedConfig(providerId: string): IProviderConfig | undefined {
    ...
    private setCachedConfig(providerId: string, config: IProviderConfig): void {
    ...
    public setProviderDefaults(providerId: string, defaults: Partial<IProviderConfig>): void {
    ...
    public getConfig(providerId: string): IProviderConfig {
    ...
    public setConfig(providerId: string, config: Partial<IProviderConfig>): void {
        const defaults = this.defaults.get(providerId) || DEFAULT_CONFIG;
        const existing = this.configs.get(providerId) || {};
    ...
    public getAllConfigs(): Map<string, IProviderConfig> {
        const allConfigs = new Map<string, IProviderConfig>();
