...
import { LLMRequest, LLMResponse, LLMSessionConfig, SessionState, SessionStats } from '../../llm/types';
import { LLMSessionError } from '../errors';
import { Logger } from '../utils/logger';
import { ConnectionStatus, LLMConnectionManager } from './LLMConnectionManager';
...
export class LLMSessionManager extends EventEmitter implements vscode.Disposable {
    ...
    constructor(connectionManager: LLMConnectionManager, hostManager: LLMHostManager, logger?: Logger) {
        super();
    ...
    async sendPrompt(prompt: string): Promise<LLMResponse> {
        if (!this.activeSessionId) {
            this.logger.error('No active session when sending prompt');
            throw new LLMSessionError('No active session', 'sendPrompt');
        }
    ...
    async sendPromptWithSession(sessionId: string, prompt: string): Promise<LLMResponse> {
        ...
        if (!session) {
            ...
            throw new LLMSessionError(`Session with ID ${sessionId} not found`, sessionId);
        }

        if (this.connectionManager.getConnectionStatus() !== ConnectionStatus.Connected) {
            this.logger.error('LLM provider is not connected', { sessionId });
            throw new LLMSessionError('LLM provider is not connected', sessionId);
        }

        const provider = this.connectionManager.getProvider();
        if (!provider) {
            this.logger.error('No LLM provider available', { sessionId });
            throw new LLMSessionError('No LLM provider available', sessionId);
        }
        ...
        try {
            ...
            this.emit('promptFailed', {
                sessionId,
                request,
                error: new LLMSessionError(`Failed to complete prompt: ${errorMessage}`, sessionId)
            });
    ...
    setSessionTimeout(timeoutMs: number): void {
        if (timeoutMs < 60000) { // Minimum 1 minute
            this.logger.error('Session timeout too short', { timeoutMs });
            throw new LLMSessionError('Session timeout must be at least 60000ms (1 minute)', undefined, { timeoutMs });
        }
