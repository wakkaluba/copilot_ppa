...
const { LMStudioProvider } = require('../../llm/lmstudio-provider');
const { Config } = require('../../config');

describe('LMStudioProvider', () => {
    let lmStudioProvider;
    let axiosStub;
    ...
    describe('isAvailable', () => {
        it('should return true when LM Studio API is available', async () => {
            ...
            expect(axios.get.calledWith(`${testUrl}/models`)).to.be.true;
        });

        it('should return false when LM Studio API is not available', async () => {
            // Stub failed API response
            axios.get.rejects(new Error('Connection failed'));
    ...
    describe('getAvailableModels', () => {
        ...
        it('should return default model when API call fails', async () => {
            // Stub failed API response
            axios.get.rejects(new Error('Failed to get models'));
    ...
    describe('generateCompletion', () => {
        ...
        it('should handle errors during text completion', async () => {
            ...
            try {
                ...
                expect(error.message).to.include('Failed to generate completion');
            }
        });
    });
    ...
    describe('generateChatCompletion', () => {
        ...
        it('should handle errors during chat completion', async () => {
            ...
            try {
                ...
                expect(error).to.be.instanceOf(Error);
                expect(error.message).to.include('Failed to generate chat completion');
            }
        });
    });
    ...
    describe('streamCompletion', () => {
        ...
        it('should stream text completion when no system prompt is provided', async () => {
            ...
            mockStream.on.withArgs('data').callsFake((event, handler) => {
                dataHandler.callsFake(handler);
                return mockStream;
            });

            mockStream.on.withArgs('end').callsFake((event, handler) => {
                endHandler.callsFake(handler);
                return mockStream;
            });

            mockStream.on.withArgs('error').callsFake((event, handler) => {
        ...
        it('should handle errors during text completion streaming', async () => {
            ...
            try {
                ...
                expect(error).to.be.instanceOf(Error);
                expect(error.message).to.include('Failed to stream completion');
            }
        });
    });
    ...
    describe('streamChatCompletion', () => {
        ...
        it('should handle errors during chat completion streaming', async () => {
            ...
            try {
                ...
                expect(error).to.be.instanceOf(Error);
                expect(error.message).to.include('Failed to stream chat completion');
            }
        });
    });
