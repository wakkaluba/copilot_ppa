 ...
 */
var ModelScalingDashboardService = /** @class */ (function (_super) {
    __extends(ModelScalingDashboardService, _super);
    function ModelScalingDashboardService(logger, metricsService) {
        var _this = _super.call(this) || this;
        _this.logger = logger;
        _this.metricsService = metricsService;
        _this.dashboardState = new Map();
        _this.subscribedModels = new Set();
        _this.logger.info('ModelScalingDashboardService initialized');
     ...
     */
    ModelScalingDashboardService.prototype.setupListeners = function () {
        var _this = this;
        this.metricsListener = function (event) {
            var modelId = event.modelId, metrics = event.metrics;
            if (_this.subscribedModels.has(modelId)) {
                _this.updateDashboard(modelId, metrics);
            }
        };
     ...
     */
    ModelScalingDashboardService.prototype.showDashboard = function (modelId) {
        return __awaiter(this, void 0, void 0, function () {
            var metrics;
            return __generator(this, function (_a) {
                try {
                    this.logger.info("Showing dashboard for model: ".concat(modelId));
                    // Subscribe to updates for this model
                    this.subscribedModels.add(modelId);
                    metrics = this.metricsService.getMetricsHistory(modelId);
                    ...
                    this.emit('dashboardOpened', { modelId: modelId });
                }
                catch (error) {
                    this.logger.error("Error showing dashboard for model ".concat(modelId), error);
                    throw error;
                }
                return [2 /*return*/];
            });
     ...
     */
    ModelScalingDashboardService.prototype.updateDashboard = function (modelId, metrics) {
        try {
            var dashboard = this.dashboardState.get(modelId) || {
                modelId: modelId,
                history: []
            };
            ...
            this.logger.info("Dashboard updated for model ".concat(modelId), { timestamp: metrics.timestamp });
            this.emit('dashboardUpdated', { modelId: modelId, metrics: metrics });
        }
        catch (error) {
            this.logger.error("Error updating dashboard for model ".concat(modelId), error);
        }
    };
    /**
     * Get current dashboard state
     */
    ModelScalingDashboardService.prototype.getDashboard = function (modelId) {
        return this.dashboardState.get(modelId);
    };
    /**
     * Close dashboard for a model
     */
    ModelScalingDashboardService.prototype.closeDashboard = function (modelId) {
        this.subscribedModels.delete(modelId);
        this.emit('dashboardClosed', { modelId: modelId });
    };
    /**
     * Dispose of resources
     */
    ModelScalingDashboardService.prototype.dispose = function () {
    ...
    return ModelScalingDashboardService;
