...
export class ComplexityAnalyzer {
     ...
     */
    private async analyzeJSComplexity(filePath: string, workspacePath: string): Promise<void> {
        try {
            ...
            const panel = vscode.window.createWebviewPanel(
                'complexityReport',
                `Complexity Report: ${path.basename(filePath)}`,
     ...
     */
    private async analyzePythonComplexity(filePath: string, workspacePath: string): Promise<void> {
        try {
     ...
     */
    private displayComplexityReport(report: any): void {
        this.outputChannel.appendLine('\nComplexity Report:');
     ...
     */
    private displayPythonComplexityReport(report: any): void {
     ...
     */
    private displayPythonMaintainabilityReport(report: any): void {
     ...
     */
    private displayPythonHalsteadReport(report: any): void {
        for (const [file, metrics] of Object.entries(report)) {
            this.outputChannel.appendLine(`\nFile: ${file}`);
            
            if (typeof metrics === 'object') {
                const hal = metrics as any;
    ...
    private getComplexityGrade(complexity: number): string {
        if (complexity <= 5) return 'A (low)';
        if (complexity <= 10) return 'B (moderate)';
        if (complexity <= 20) return 'C (high)';
        if (complexity <= 30) return 'D (very high)';
     ...
     */
    private getMaintainabilityRating(mi: number): string {
        if (mi >= 20) return 'A (high)';
        if (mi >= 10) return 'B (moderate)';
        return 'C (low)';
    }
     ...
     */
    private getComplexityReportHtml(filePath: string, reportData: string): string {
        let report;
        try {
     ...
     */
    private renderComplexityReportHtml(report: any): string {
        if (report.raw) {
        ...
        report.forEach(module => {
            ...
            if (module.functions && module.functions.length > 0) {
                ...
                module.functions.forEach((func: any) => {
                    ...
                    if (complexity > 5) complexityClass = 'complexity-b';
                    if (complexity > 10) complexityClass = 'complexity-c';
                    if (complexity > 20) complexityClass = 'complexity-d';
                    if (complexity > 30) complexityClass = 'complexity-e';
     ...
     */
    private getPythonComplexityReportHtml(filePath: string, ccReport: any, miReport: any, halReport: any): string {
        return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
     ...
     */
    private renderPythonCyclomaticComplexity(report: any): string {
        let html = '';
     ...
     */
    private renderPythonMaintainability(report: any): string {
        let html = `<table>
            <tr>
                <th>File</th>
        ...
        for (const [file, mi] of Object.entries(report)) {
            ...
            if (miValue < 20) miClass = 'complexity-b';
            if (miValue < 10) miClass = 'complexity-c';
     ...
     */
    private renderPythonHalstead(report: any): string {
        let html = `<table>
            <tr>
                <th>File</th>
                <th>Vocabulary</th>
