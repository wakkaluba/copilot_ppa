import { LLMModel } from '../llmProviders/llmModels';

// Test fixtures with improved typing
const createBasicModel = (): LLMModel => ({
    id: 'test-model',
    name: 'Test Model',
    provider: 'ollama',
...
const createFullModel = (): LLMModel => ({
    ...createBasicModel(),
    parameters: {
        temperature: 0.7,
        maxTokens: 2048
...
describe('LLMModel Interface', () => {
    describe('Required Properties', () => {
        ...
        it('should error when missing required properties', () => {
            const requiredProps = ['id', 'name', 'provider', 'description'] as const;
            requiredProps.forEach(prop => {
                const invalidModel = { ...createBasicModel() };
                delete (invalidModel as any)[prop];
    ...
    describe('Provider Types', () => {
        it('should accept all valid provider types', () => {
            const providers = ['ollama', 'lmstudio', 'huggingface'] as const;
    ...
    describe('Optional Properties', () => {
        ...
        it('should validate optional property types when present', () => {
            const invalidModels = [
                { ...createBasicModel(), parameters: 'invalid' as any },
                { ...createBasicModel(), size: 123 as any },
                { ...createBasicModel(), license: true as any },
                { ...createBasicModel(), tags: 'invalid' as any },
                { ...createBasicModel(), installed: 'yes' as any }
            ] as LLMModel[];
...
// Validation helpers
function validateModel(model: LLMModel): void {
    // Required properties
    const requiredProps = ['id', 'name', 'provider', 'description'] as const;
    requiredProps.forEach(prop => {
        if (!model[prop]) {
...
function validateOllamaSpecificRequirements(model: LLMModel): void {
    expect(model.provider).toBe('ollama');
    expect(model.id).not.toContain('/');
    validateModel(model);
}

function validateLMStudioSpecificRequirements(model: LLMModel): void {
    expect(model.provider).toBe('lmstudio');
    expect(model.id).toContain('/');
    validateModel(model);
}

function validateHuggingFaceSpecificRequirements(model: LLMModel): void {
