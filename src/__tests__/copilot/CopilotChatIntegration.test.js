...
const { CopilotChatIntegration } = require('../../copilot/copilotChatIntegration');
const { CopilotApiService } = require('../../services/copilotApi');
const { Logger } = require('../../utils/logger');

// Mock dependencies
jest.mock('../../services/copilotApi');
jest.mock('../../utils/logger');
jest.mock('../../copilot/services/CopilotChatInitializationService', () => {
  return {
...
jest.mock('../../copilot/services/CopilotChatParticipantService', () => {
  return {
...
jest.mock('../../copilot/services/CopilotChatMessageHandlerService', () => {
  return {
...
jest.mock('../../copilot/services/CopilotChatCommandHandlerService', () => {
  return {
...
describe('CopilotChatIntegration (JavaScript)', () => {
  ...
  beforeEach(() => {
    ...
    // Accessing private static property for testing
    ...
    mockInitService = jest.requireMock('../../copilot/services/CopilotChatInitializationService');
    mockParticipantService = jest.requireMock('../../copilot/services/CopilotChatParticipantService');
    mockMessageHandlerService = jest.requireMock('../../copilot/services/CopilotChatMessageHandlerService');
    mockCommandHandlerService = jest.requireMock('../../copilot/services/CopilotChatCommandHandlerService');
  ...
  it('should fail initialization when extension initialization fails', async () => {
    ...
    initServiceInstance.initializeCopilotExtension = jest.fn().mockResolvedValue(null);
  ...
  it('should handle errors in chat message processing', async () => {
    const instance = CopilotChatIntegration.getInstance();
    await instance.initialize();

    // Setup mock to throw error
    const messageHandlerServiceInstance = mockMessageHandlerService.CopilotChatMessageHandlerService.mock.results[0].value;
    messageHandlerServiceInstance.handleMessage = jest.fn().mockRejectedValue(new Error('Handler error'));
    messageHandlerServiceInstance.createErrorResponse = jest.fn().mockReturnValue({ error: 'Formatted error' });
