// filepath: d:\___coding\tools\copilot_ppa\src\__tests__\diagnostics\DiagnosticReportGenerator.test.ts
import { afterEach, beforeEach, describe, expect, jest, test } from '@jest/globals';
import * as fs from 'fs';
import * as vscode from 'vscode';
import { DiagnosticReportGenerator, IDiagnosticReportContent } from '../../diagnostics/diagnosticReport';
...
describe('DiagnosticReportGenerator', () => {
  ...
  let mockSystemChecker: jest.Mocked<SystemRequirementsChecker>;
  ...
  beforeEach(() => {
    ...
    mockSystemChecker = new SystemRequirementsChecker() as jest.Mocked<SystemRequirementsChecker>;
    mockContext = {
      subscriptions: [],
  ...
  test('constructor should create output channel and initialize services', () => {
    ...
    expect((diagnosticReportGenerator as any)._logger).toBe(mockLogger);
    expect((diagnosticReportGenerator as any)._extensionContext).toBe(mockContext);
    expect((diagnosticReportGenerator as any)._systemChecker).toBe(mockSystemChecker);

    // Test that services were initialized
    expect((diagnosticReportGenerator as any).systemInfoSvc).toBeDefined();
    expect((diagnosticReportGenerator as any).configSvc).toBeDefined();
    expect((diagnosticReportGenerator as any).perfSvc).toBeDefined();
    expect((diagnosticReportGenerator as any).runtimeTracker).toBeDefined();
    expect((diagnosticReportGenerator as any).logService).toBeDefined();
  });
  ...
  describe('trackRequest', () => {
    test('should track successful request', () => {
      ...
      expect((diagnosticReportGenerator as any)._responseTimeHistory).toContain(200);

      // Verify request count increased
      expect((diagnosticReportGenerator as any)._requestCount).toBe(1);

      // Verify error count did not increase
      expect((diagnosticReportGenerator as any)._errorCount).toBe(0);
    });
    ...
    test('should track error request', () => {
      ...
      // Verify the response time was added to history
      expect((diagnosticReportGenerator as any)._responseTimeHistory).toContain(300);

      // Verify request count increased
      expect((diagnosticReportGenerator as any)._requestCount).toBe(1);

      // Verify error count increased
      expect((diagnosticReportGenerator as any)._errorCount).toBe(1);

      // Verify error message was stored
      expect((diagnosticReportGenerator as any)._lastError).toBe('Test error message');

      // Verify error time was stored
      expect((diagnosticReportGenerator as any)._lastErrorTime).not.toBeNull();
    });
    ...
    test('should limit response time history to 100 entries', () => {
      ...
      expect((diagnosticReportGenerator as any)._responseTimeHistory.length).toBe(100);

      // Verify oldest entries were removed (10 entries removed, so first entry should be 110)
      expect((diagnosticReportGenerator as any)._responseTimeHistory[0]).toBe(110);
    });
  });
  ...
  describe('generateReport', () => {
    test('should generate a valid diagnostic report', async () => {
      ...
      expect((diagnosticReportGenerator as any).systemInfoSvc.collect).toHaveBeenCalled();
      expect((diagnosticReportGenerator as any).configSvc.flatten).toHaveBeenCalled();
      expect((diagnosticReportGenerator as any).perfSvc.getMetrics).toHaveBeenCalled();
      expect((diagnosticReportGenerator as any).runtimeTracker.getInfo).toHaveBeenCalled();
      expect((diagnosticReportGenerator as any).logService.getRecent).toHaveBeenCalled();
    ...
    test('should handle errors during report generation', async () => {
      ...
      (diagnosticReportGenerator as any).systemInfoSvc.collect.mockRejectedValueOnce(new Error('Test error'));
  ...
  describe('saveReportToFile', () => {
    test('should save report to a file', async () => {
      ...
      const fileContent = (fs.writeFileSync as jest.Mock).mock.calls[0][1];
