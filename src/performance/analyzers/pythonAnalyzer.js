...
var PythonAnalyzer = /** @class */ (function (_super) {
    ...
    PythonAnalyzer.prototype.analyze = function (fileContent, filePath) {
        ...
        var metrics = this.calculatePythonMetrics(fileContent);
        result.metrics = __assign(__assign({}, result.metrics), metrics);
        return result;
    };
    PythonAnalyzer.prototype.analyzeMemoryPatterns = function (fileContent, lines, result) {
    ...
    PythonAnalyzer.prototype.analyzePerformancePatterns = function (fileContent, lines, result) {
    ...
    PythonAnalyzer.prototype.analyzeComprehensions = function (fileContent, lines, result) {
        // Check for nested loops that could be list comprehensions
        var nestedLoopRegex = /for\s+\w+\s+in\s+.+:\s*\n\s+for\s+\w+\s+in\s+.+:\s*\n\s+\w+\.append/g;
        var match;
    ...
    PythonAnalyzer.prototype.analyzeGeneratorUsage = function (fileContent, lines, result) {
        // Check for opportunities to use generators
        var listCreationRegex = /list\s*\(\s*range\s*\(\s*\d+\s*\)\s*\)/g;
        var match;
    ...
    PythonAnalyzer.prototype.analyzeDataStructures = function (fileContent, lines, result) {
    ...
    PythonAnalyzer.prototype.analyzePythonSpecifics = function (fileContent, lines, result) {
        // Check for missing type hints
        var noTypeHints = /def\s+\w+\s*\([^:)]*\)\s*:/g;
        var match;
    ...
    PythonAnalyzer.prototype.calculatePythonMetrics = function (content) {
        return {
