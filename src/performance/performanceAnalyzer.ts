...
export class PerformanceAnalyzer {
    ...
    private analysisResults: Map<string, PerformanceAnalysisResult>;
     ...
     */
    public async analyzeActiveFile(): Promise<PerformanceAnalysisResult | null> {
    ...
    public async analyzeWorkspace(): Promise<WorkspacePerformanceResult> {
        ...
        let mediumIssues = 0;
        let lowIssues = 0;
        const fileResults: PerformanceAnalysisResult[] = [];
    ...
    public showFileAnalysisReport(result: PerformanceAnalysisResult): void {
        const panel = vscode.window.createWebviewPanel(
            'performanceAnalysis',
            `Performance Analysis: ${path.basename(result.filePath)}`,
     ...
     */
    private generateFileReportHtml(result: PerformanceAnalysisResult): string {
     ...
     */
    private generateWorkspaceReportHtml(result: WorkspacePerformanceResult): string {
     ...
     */
    private identifyCommonIssueTypes(fileResults: PerformanceAnalysisResult[]): { [key: string]: any } {
        const issueTypes: { [key: string]: { count: number, files: string[], severity: string, description: string, solution: string } } = {};
     ...
     */
    private isCacheValid(cachedResult: PerformanceAnalysisResult, currentContent: string): boolean {
     ...
     */
    private getMetricRating(key: string, value: number): string {
     ...
     */
    private analyzeJavaScript(fileContent: string, filePath: string): PerformanceAnalysisResult {
     ...
     */
    private calculateJavaScriptMetrics(fileContent: string): { [key: string]: number } {
     ...
     */
    private estimateAverageFunctionLength(content: string): number {
     ...
     */
    private estimateMaxNestedDepth(content: string): number {
     ...
     */
    private estimateAverageParameterCount(content: string): number {
     ...
     */
    private analyzePython(fileContent: string, filePath: string): PerformanceAnalysisResult {
        const issues: PerformanceIssue[] = [];
        const fileHash = this.calculateContentHash(fileContent);
        const lines = fileContent.split('\n');
     ...
     */
    private calculateAveragePythonFunctionLength(content: string): number {
     ...
     */
    private calculatePythonNestedDepth(content: string): number {
        const lines = content.split('\n');
        let maxDepth = 0;
        let currentDepth = 0;
     ...
     */
    private analyzeJava(fileContent: string, filePath: string): PerformanceAnalysisResult {
        const issues: PerformanceIssue[] = [];
        const fileHash = this.calculateContentHash(fileContent);
        const lines = fileContent.split('\n');
     ...
     */
    private calculateJavaAverageMethodLength(content: string): number {
        const methodMatches = content.match(/\b(public|private|protected)\s+\w+\s+\w+\s*\([^{]*\{(?:\{[^}]*\}|[^}])*?\n\s*\}/g) || [];
        if (methodMatches.length === 0) return 0;
     ...
     */
    private analyzeCSharp(fileContent: string, filePath: string): PerformanceAnalysisResult {
        const issues: PerformanceIssue[] = [];
        const fileHash = this.calculateContentHash(fileContent);
        const lines = fileContent.split('\n');
     ...
     */
    private calculateCSharpAverageMethodLength(content: string): number {
        const methodMatches = content.match(/\b(public|private|protected)\s+[\w<>]+\s+\w+\s*\([^{]*\{(?:\{[^}]*\}|[^}])*?\n\s*\}/g) || [];
        if (methodMatches.length === 0) return 0;
     ...
     */
    private analyzeGeneric(fileContent: string, filePath: string): PerformanceAnalysisResult {
        const issues: PerformanceIssue[] = [];
        const fileHash = this.calculateContentHash(fileContent);
        const lines = fileContent.split('\n');
     ...
     */
    private extractCodeSnippet(lines: string[], centerLine: number, contextLines: number): string {
     ...
     */
    private extractFunctionBody(content: string, position: number): string {
        let bracketCount = 0;
