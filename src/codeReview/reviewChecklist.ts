...
export class ReviewChecklist {
    private readonly service: ReviewChecklistService;
    ...
    private validateChecklistItems(items: ChecklistItem[]): void {
        if (!Array.isArray(items) || items.length === 0) {
            throw new ReviewChecklistError('Checklist items must be a non-empty array');
        }
        items.forEach(item => {
            if (!item.id || !item.description) {
                throw new ReviewChecklistError('Each checklist item must have an id and description');
            }
        });
    }

    private validateResults(results: ReviewResult[]): void {
        if (!Array.isArray(results)) {
            throw new ReviewChecklistError('Results must be an array');
        }
        results.forEach(result => {
            if (!result.itemId || typeof result.passed !== 'boolean') {
                throw new ReviewChecklistError('Each result must have an itemId and passed status');
            }
        });
    }

    private createEmptyReport(checklistName: string, filePaths: string[], reviewerId: string): Report {
        return {
            id: `error-${Date.now()}`,
            checklistName,
            filePaths,
    ...
    private createErrorReport(reportId: string): string {
        return `
            <html>
                <body>
    ...
    private handleError(message: string, error: unknown): void {
        ...
        throw new ReviewChecklistError(`${message}: ${errorMessage}`);
    }
}
