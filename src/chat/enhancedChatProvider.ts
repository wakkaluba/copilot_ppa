...
import { ContextManager, ConversationMessage } from '../contextManager';
...
export class EnhancedChatProvider {
                ...
                <div class="input-container">
                    <div class="suggestions-container">
                        ${suggestions.map(suggestion => 
                            `<div class="suggestion-chip" data-suggestion="${this.escapeHtml(suggestion)}">${this.escapeHtml(suggestion)}</div>`
                        ).join('')}
                    </div>
     ...
     */
    private async generateResponse(userMessage: ConversationMessage): Promise<void> {
        try {
     ...
     */
    private addMessageToUI(message: ConversationMessage): void {
        if (this.view) {
     ...
     */
    private addSystemMessageToUI(content: string): void {
        if (this.view) {
            this.view.webview.postMessage({
                command: 'addMessage',
                id: 'system-' + Date.now(),
     ...
     */
    private removeSystemMessageFromUI(): void {
     ...
     */
    private renderMessages(messages: ConversationMessage[]): string {
