...
export class CodeExampleSearch {
     ...
     */
    private async searchGitHub(query: string, language: string): Promise<any[]> {
        ...
        const contentPromises = items.map(async (item: any) => {
            try {
     ...
     */
    private filterByRelevance(results: any[], query: string): CodeExample[] {
     ...
     */
    private getCachedResult(cacheKey: string): CodeExample[] | null {
     ...
     */
    private cacheResults(cacheKey: string, results: CodeExample[]): void {
        const cacheFilePath = path.join(this.cachePath, `${cacheKey}.json`);
        const cacheData = {
     ...
     */
    private generateExampleHTML(examples: CodeExample[], webview: vscode.Webview): string {
