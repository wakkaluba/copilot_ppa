...
export class ApiDocumentationGenerator {
     ...
     */
    private async generateApiDocForFile(): Promise<void> {
        try {
            ...
            if (apiDoc) {
                ...
                const saveAction = await vscode.window.showInformationMessage(
                    'API documentation generated. Do you want to save it?',
                    'Save',
                    'Save As',
                    'No'
                );
     ...
     */
    private async generateApiDocForProject(): Promise<void> {
        try {
...
<body>
    ...
    private async generateOpenApiSpec(): Promise<void> {
        try {
            ...
            const prompt = this.createOpenApiPrompt(
                combinedApiCode,
                apiInfo,
                selectedVersion,
                selectedFormat
            ...
            if (openApiSpec) {
                ...
                if (fs.existsSync(outputPath)) {
                    const action = await vscode.window.showWarningMessage(
                        `${outputFileName} already exists. What would you like to do?`,
                ...
                const previewAction = await vscode.window.showInformationMessage(
                    'Would you like to preview the OpenAPI specification in Swagger UI?',
                    'Yes', 'No'
                );
    ...
    private async getProjectSourceFiles(projectRoot: string): Promise<string[]> {
        ...
        const includeJs = config.get<boolean>('includeJavaScript', true);
        const includeTs = config.get<boolean>('includeTypeScript', true);
        const includePy = config.get<boolean>('includePython', false);
        const includeJava = config.get<boolean>('includeJava', false);
        const includeCs = config.get<boolean>('includeCSharp', false);
        const includeGo = config.get<boolean>('includeGo', false);
        
        const patterns: string[] = [];
        
        if (includeJs) patterns.push('**/*.js', '**/*.jsx');
        if (includeTs) patterns.push('**/*.ts', '**/*.tsx');
        if (includePy) patterns.push('**/*.py');
        if (includeJava) patterns.push('**/*.java');
        if (includeCs) patterns.push('**/*.cs');
        if (includeGo) patterns.push('**/*.go');
     ...
     */
    private async findApiFiles(projectRoot: string): Promise<string[]> {
     ...
     */
    private async collectApiInfo(): Promise<any | undefined> {
     ...
     */
    private createApiDocPrompt(code: string, fileExt: string, format: string): string {
...
Generate ONLY the ${version} specification in ${format} format, without additional explanations.`;
     ...
     */
    private getFileExtensionForFormat(format: string): string {
     ...
     */
    private getLanguageIdForFormat(format: string): string {
        switch (format.toLowerCase()) {
            case ApiDocFormat.HTML:
                return 'html';
            case ApiDocFormat.JSON:
     ...
     */
    private generateSwaggerUiHtml(specFile: string): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
