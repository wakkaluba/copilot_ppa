...
describe('LocalizationService', () => {
    ...
    describe('normalizeLanguage', () => {
        it('should extract base language from locale format', () => {
            ...
            service.setLanguage('en' as SupportedLanguage); // Using the private method through a public one
            expect(service.getCurrentLanguage()).toBe('en');
        });
    ...
    describe('detectLanguage', () => {
        ...
        it('should detect Japanese text', () => {
            expect(service.detectLanguage('これは日本語です')).toBe('ja');
        });

        it('should detect Chinese text', () => {
            expect(service.detectLanguage('这是中文')).toBe('zh');
        });

        it('should detect Korean text', () => {
            expect(service.detectLanguage('이것은 한국어입니다')).toBe('ko');
        });

        it('should detect Russian text', () => {
            expect(service.detectLanguage('Это русский текст')).toBe('ru');
        });

        it('should detect Ukrainian text', () => {
            expect(service.detectLanguage('Це український текст з літерами і є')).toBe('uk');
        });

        it('should detect Arabic text', () => {
            expect(service.detectLanguage('هذا نص باللغة العربية')).toBe('ar');
        });

        it('should detect Thai text', () => {
            expect(service.detectLanguage('นี่คือข้อความภาษาไทย')).toBe('th');
        });

        it('should detect Greek text', () => {
            expect(service.detectLanguage('Αυτό είναι ελληνικό κείμενο')).toBe('el');
        });
        ...
        it('should detect language from a mix of text with stronger patterns', () => {
            // Text with a mix of languages but with stronger Japanese pattern
            expect(service.detectLanguage('This is a テスト of mixed text')).toBe('ja');

            // Text with a mix of languages but with stronger Ukrainian pattern
            expect(service.detectLanguage('This contains the Ukrainian letter Є')).toBe('uk');
        });

        it('should correctly identify language with scriptio continua', () => {
            expect(service.detectLanguage('これは単語の区切りがない日本語です')).toBe('ja');
            expect(service.detectLanguage('这是没有词间隔的中文文本')).toBe('zh');
        });
    });
    ...
    describe('integration with locale files', () => {
        it('should correctly load and use locale files from extension path', () => {
            // Setup real extension path mocking
            const mockExtensionPath = '/test/extension/path';
            mockContext.extensionPath = mockExtensionPath;

            // Setup fake locales directory and files
            const enLocalePath = `${mockExtensionPath}/locales/en.json`;
            const deLocalePath = `${mockExtensionPath}/locales/de.json`;

            // Mock file existence
            fsExistsStub.callsFake((path: string) => {
                return path === enLocalePath || path === deLocalePath;
            });

            // Mock file content
            fsReadFileStub.callsFake((path: string) => {
                if (path === enLocalePath) {
                    return JSON.stringify({
                        'greeting': 'Hello',
                        'farewell': 'Goodbye'
                    });
                } else if (path === deLocalePath) {
                    return JSON.stringify({
                        'greeting': 'Hallo',
            ...
            // Test initial loading with English
            envLanguageStub.value('en');
            const service = new LocalizationService(mockContext);
            expect(service.getString('greeting', 'default')).toBe('Hello');
            expect(service.getString('farewell', 'default')).toBe('Goodbye');

            // Test switching to German
            service.setLanguage('de');
            expect(service.getString('greeting', 'default')).toBe('Hallo');
            expect(service.getString('farewell', 'default')).toBe('Auf Wiedersehen');
        });
    });
});
