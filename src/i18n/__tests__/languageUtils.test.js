// filepath: d:\___coding\tools\copilot_ppa\src\i18n\__tests__\languageUtils.test.js
// @ts-nocheck
const {
    languageNames,
    getLanguageName,
...
describe('languageUtils (JavaScript)', () => {
    describe('languageNames', () => {
        ...
        it('should map language codes to correct language names', () => {
            ...
            for (const [code, name] of Object.entries(expectedMappings)) {
                expect(languageNames.get(code)).toBe(name);
            }
        });
    });
    ...
    describe('isKnownLanguage', () => {
        ...
        it('should work with dynamic properties in JavaScript', () => {
            const someLanguage = 'en';
            if (isKnownLanguage(someLanguage)) {
                const name = getLanguageName(someLanguage);
