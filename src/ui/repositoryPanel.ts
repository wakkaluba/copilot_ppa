...
export class RepositoryPanel {
            ...
            async message => {
                switch (message.command) {
                    ...
                    case 'toggleRepositoryAccess':
                        repositoryManager.setEnabled(message.enabled);
                        vscode.window.showInformationMessage(
                            `Repository access ${message.enabled ? 'enabled' : 'disabled'}`
                        );
                        return;
                }
            },
    ...
    private _getHtmlForWebview(extensionUri: vscode.Uri) {
        // Get the providers
        const providers = repositoryManager.getProviders();
        ...
        <body>
            ...
            <div class="toggle-container">
                <label class="toggle-switch">
                    <input type="checkbox" id="repoAccess" ${repositoryManager.isEnabled ? 'checked' : ''}>
                    <span class="slider"></span>
                </label>
                <span>Repository Access: ${repositoryManager.isEnabled ? 'Enabled' : 'Disabled'}</span>
            </div>

            <div id="repoForm" ${!repositoryManager.isEnabled ? 'style="opacity: 0.5; pointer-events: none;"' : ''}>
