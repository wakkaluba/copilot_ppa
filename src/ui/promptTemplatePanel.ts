...
import { getPromptTemplateManager } from '../services/promptTemplates/manager';
import { PromptTemplate, NewPromptTemplate } from '../services/promptTemplates/model';
import { PromptTemplateHtmlProvider } from './PromptTemplateHtmlProvider';

export class PromptTemplatePanel {
    public static readonly viewType = 'copilotPPA.promptTemplatePanel';
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri) {
        ...
        this._panel.webview.onDidReceiveMessage(
            async message => {
                const templateManager = getPromptTemplateManager();
                ...
                switch (message.command) {
                    ...
                    case 'createTemplate':
                        try {
                            const newTemplate = message.template as NewPromptTemplate;
                        ...
                        try {
                            ...
                            if (fileUri && fileUri.length > 0) {
                                ...
                                vscode.window.showInformationMessage(
                                    `Imported ${result.success} templates (${result.failed} failed)`
                                );
    ...
    private _update() {
        ...
        this._panel.webview.html = PromptTemplateHtmlProvider.getHtml(this._panel.webview.uri);
    }

    public show(uri: vscode.Uri): void {
        const column = vscode.window.activeTextEditor ? vscode.window.activeTextEditor.viewColumn : undefined;
        const panel = this._panel || vscode.window.createWebviewPanel(
            'promptTemplate',
            'Prompt Template',
            column || vscode.ViewColumn.One,
            { enableScripts: true }
        ...
        this._panel = panel;
        panel.webview.html = PromptTemplateHtmlProvider.getHtml(uri);
        panel.webview.onDidReceiveMessage(
            message => this.handleMessage(message, uri),
            undefined,
            this._disposables
        );
    }
