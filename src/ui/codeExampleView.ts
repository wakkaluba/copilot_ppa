...
import { CodeExample, CodeExampleService } from '../services/codeExamples/codeExampleService';
...
export class CodeExampleViewProvider implements vscode.WebviewViewProvider {
     ...
     */
    private insertCodeExample(code: string): void {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
     ...
     */
    private extractKeywords(editor?: vscode.TextEditor): string[] {
        if (!editor) {
            return [];
        }
        ...
        const uniqueKeywords = [...new Set(allMatches)].filter(
            keyword => !commonKeywords.includes(keyword) && keyword.length > 2
        );
    ...
    private _getHtmlForWebview(webview: vscode.Webview): string {
        ...
        const styleMainUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, 'media', 'codeExamples.css')
        );

        // Get scripts
        const scriptUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, 'media', 'codeExamples.js')
        );
