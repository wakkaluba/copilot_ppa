...
ONLY suggest commands that are relevant to the task and appropriate for the specified shell.
     ...
     */
    public async analyzeFailedCommand(
        command: string,
        error: string,
...
Only suggest realistic fixes based on the error message and command context.
     ...
     */
    public async generateCommandFromDescription(
        description: string,
        shellType: TerminalShellType,
        includeContextualInfo: boolean = true
...
`;
     ...
     */
    private async buildContextInformation(workspacePath: string): Promise<string> {
        if (!workspacePath) {
            return '';
        }
     ...
     */
    private parseCommandGenerationResponse(response: string): CommandGenerationResult {
     ...
     */
    private validateCommandSyntax(command: string, shellType: TerminalShellType): boolean {
     ...
     */
    public async analyzeCommand(command: string, shellType: TerminalShellType): Promise<CommandAnalysis> {
        try {
            const prompt = `
...
`;
     ...
     */
    private parseCommandAnalysis(response: string): CommandAnalysis {
     ...
     */
    public async generateCommandVariations(
        baseCommand: string,
        description: string,
        shellType: TerminalShellType
    ): Promise<string[]> {
...
`;
     ...
     */
    private parseCommandVariations(response: string): string[] {
     ...
     */
    private parseCommandSuggestions(response: string): string[] {
     ...
     */
    private parseFixSuggestions(response: string): string[] {
        const fixes: string[] = [];
     ...
     */
    private cleanGeneratedCommand(response: string): string {
     ...
     */
    private formatCommandHistoryForPrompt(shellType: TerminalShellType): string {
