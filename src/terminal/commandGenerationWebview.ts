...
import { AITerminalHelper, CommandGenerationResult, CommandAnalysis } from './aiTerminalHelper';
...
export class CommandGenerationWebview {
    ...
    private shellType: TerminalShellType = TerminalShellType.VSCodeDefault;
    private currentCommand: string = '';
    private lastAnalysis: CommandAnalysis | null = null;
    
    constructor(
        context: vscode.ExtensionContext,
        aiHelper: AITerminalHelper,
     ...
     */
    private getWebviewContent(shellType: TerminalShellType, initialPrompt: string = ''): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
     ...
     */
    private async generateCommand(description: string): Promise<void> {
        try {
            const result = await this.aiHelper.generateCommandFromDescription(
                description,
                this.shellType,
                true
            );
     ...
     */
    private async runCommand(terminalCommand: string): Promise<void> {
        try {
     ...
     */
    private async analyzeCommand(terminalCommand: string): Promise<void> {
        try {
     ...
     */
    private async generateVariations(terminalCommand: string, description: string): Promise<void> {
        try {
            const variations = await this.aiHelper.generateCommandVariations(
                terminalCommand,
                description,
                this.shellType
            );
