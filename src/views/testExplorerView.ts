...
export class TestExplorerProvider implements vscode.TreeDataProvider<TestItem> {
    ...
    getChildren(element?: TestItem): Thenable<TestItem[]> {
        ...
        if (result) {
            ...
            // Add status item
            const statusItem = new TestItem(
                `Status: ${result.success ? 'Passed' : 'Failed'}`,
                `${element.testType}-status`,
            ...
            // Add message item
            const messageItem = new TestItem(
                `Message: ${result.message}`,
                `${element.testType}-message`,
                vscode.TreeItemCollapsibleState.None
            );
            ...
            if (element.testType === 'performance' && result.performanceMetrics) {
                const metricsItem = new TestItem(
                    'Performance Metrics',
                    `${element.testType}-metrics`,
                ...
                metricsItem.children = Object.entries(result.performanceMetrics).map(([key, value]) => {
                    const metricItem = new TestItem(
                        `${this.formatMetricName(key)}: ${value}`,
                        `${element.testType}-metric-${key}`,
                        vscode.TreeItemCollapsibleState.None
                    );
                    return metricItem;
                });
            ...
            if (element.testType === 'static' && result.staticAnalysis) {
                ...
                const issuesItem = new TestItem(
                    `Issues: ${issueCount}`,
                    `${element.testType}-issues`,
                    issueCount > 0 ? vscode.TreeItemCollapsibleState.Expanded : vscode.TreeItemCollapsibleState.None
                );
                ...
                if (result.staticAnalysis.issues && result.staticAnalysis.issues.length > 0) {
                    issuesItem.children = result.staticAnalysis.issues.map((issue, index) => {
                        ...
                        const issueItem = new TestItem(
                            `${severity} ${location}: ${issue.message}`,
                            `${element.testType}-issue-${index}`,
                            vscode.TreeItemCollapsibleState.None
                        );
            ...
            if (element.testType === 'coverage' && result.codeCoverage) {
                ...
                const overallItem = new TestItem(
                    `Overall: ${coverage.overall.toFixed(2)}%`,
                    `${element.testType}-overall`,
                    vscode.TreeItemCollapsibleState.None
                );
                ...
                if (coverage.files && coverage.files.length > 0) {
                    const filesItem = new TestItem(
                        `Files (${coverage.totalFiles})`,
                        `${element.testType}-files`,
                    ...
                    filesItem.children = coverage.files.map((file, index) => {
                        const fileItem = new TestItem(
                            `${file.path}: ${file.overall.toFixed(2)}%`,
                            `${element.testType}-file-${index}`,
                            vscode.TreeItemCollapsibleState.None
                        );
            ...
            if (element.testType === 'security' && result.securityTest) {
                ...
                // Add summary item
                const summaryItem = new TestItem(
                    `Vulnerabilities: ${summary.total}`,
                    `${element.testType}-summary`,
                    vscode.TreeItemCollapsibleState.Expanded
                );
                items.push(summaryItem);
                
                // Add breakdown by severity
                const criticalItem = new TestItem(
                    `Critical: ${summary.critical}`,
                    `${element.testType}-critical`,
                    summary.critical > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
                );
                criticalItem.iconPath = summary.critical > 0 
                    ? new vscode.ThemeIcon('error', new vscode.ThemeColor('testing.iconFailed'))
                    : new vscode.ThemeIcon('pass-filled', new vscode.ThemeColor('testing.iconPassed'));
                
                const highItem = new TestItem(
                    `High: ${summary.high}`,
                    `${element.testType}-high`,
                    summary.high > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
                );
                highItem.iconPath = summary.high > 0 
                    ? new vscode.ThemeIcon('warning', new vscode.ThemeColor('testing.iconFailed'))
                    : new vscode.ThemeIcon('pass-filled', new vscode.ThemeColor('testing.iconPassed'));
                
                const mediumItem = new TestItem(
                    `Medium: ${summary.medium}`,
                    `${element.testType}-medium`,
                    summary.medium > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
                );
                mediumItem.iconPath = summary.medium > 0 
                    ? new vscode.ThemeIcon('warning', new vscode.ThemeColor('testing.iconSkipped'))
                    : new vscode.ThemeIcon('pass-filled', new vscode.ThemeColor('testing.iconPassed'));
                
                const lowItem = new TestItem(
                    `Low: ${summary.low}`,
                    `${element.testType}-low`,
                    summary.low > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
                );
                lowItem.iconPath = summary.low > 0 
                    ? new vscode.ThemeIcon('info', new vscode.ThemeColor('testing.iconQueued'))
                    : new vscode.ThemeIcon('pass-filled', new vscode.ThemeColor('testing.iconPassed'));
                
                const infoItem = new TestItem(
                    `Info: ${summary.info}`,
                    `${element.testType}-info`,
                    summary.info > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
                );
     ...
     */
    private setCoverageIcon(item: TestItem, coverage: number): void {
        if (coverage >= 80) {
     ...
     */
    private createVulnerabilityItem(vuln: any, id: string): TestItem {
