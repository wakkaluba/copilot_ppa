...
import { TestRunnerService } from '../services/testRunner/testRunnerService';
import { TestRunnerOptions } from '../services/testRunner/testRunnerTypes';
import * as path from 'path';
import { SecurityVulnerabilityPanel } from '../views/securityVulnerabilityPanel';

/**
 * Registers all test runner commands with VS Code
 */
export function registerTestRunnerCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runUnitTests', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runIntegrationTests', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runE2ETests', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
            ...
            const configureOption = await vscode.window.showQuickPick(
                ['Run with auto-detected settings', 'Configure E2E test settings'],
                { placeHolder: 'How do you want to run E2E tests?' }
            );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runPerformanceTests', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runStaticAnalysis', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runCodeCoverage', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
            ...
            vscode.window.withProgress({
                ...
                if (result.codeCoverage) {
                    ...
                    const enableHighlighting = await vscode.window.showInformationMessage(
                        'Enable coverage highlighting in the editor?',
                        'Yes', 'No'
                    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.runSecurityTest', async () => {
            // Get workspace folder or let user select one if multiple folders are open
            const workspaceFolder = await getWorkspaceFolder();
            if (!workspaceFolder) {
                return;
            ...
            vscode.window.withProgress({
                ...
                const options: SecurityTestOptions = {
                    ...
                    tool: tool as any,
                    command,
                    severityThreshold: severityThreshold?.value as any,
                    failOnVulnerabilities: failOnVulnerabilities?.value as boolean
                };
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.viewSecurityVulnerabilities', (vulnerabilities) => {
            SecurityVulnerabilityPanel.createOrShow(
                context.extensionUri, 
                vulnerabilities,
                'Security Vulnerabilities'
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('localLLMAgent.openFileAtLocation', async (filePath, line, column) => {
            try {
