...
module.exports = function transformer(fileInfo, api, options) {
    ...
    if (!root.find(j.ImportDeclaration).filter(path => path.node.source.value === 'jest').size()) {
        root.get().node.program.body.unshift(j.importDeclaration([j.importDefaultSpecifier(j.identifier('jest'))], j.literal('jest')));
    }
    ...
    root.find(j.MemberExpression)
        ...
        if (path.parent.node.type === 'CallExpression') {
            ...
            j(path.parent).replaceWith(j.callExpression(j.memberExpression(j.identifier('getCachedMock'), j.identifier('call')), [
                j.literal(`vscode.${apiName}`),
                j.arrowFunctionExpression([], createMockImplementation(j, apiName))
            ]));
        }
    });
    return root.toSource();
};
//# sourceMappingURL=vscodeApiTransformer.js.map