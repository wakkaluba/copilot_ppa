...
class BottleneckDetector {
     ...
     */
    async detectBottlenecksInCurrentFile() {
     ...
     */
    async detectBottlenecks(fileUri) {
     ...
     */
    async analyzeWorkspaceBottlenecks() {
     ...
     */
    detectStructuralBottlenecks(content, fileType) {
        const bottlenecks = [];
     ...
     */
    detectAlgorithmicBottlenecks(content, fileType) {
        const bottlenecks = [];
        const lines = content.split('\n');
     ...
     */
    detectResourceBottlenecks(content, fileType) {
        const bottlenecks = [];
        const lines = content.split('\n');
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i].trim();
     ...
     */
    containsNestedLoop(content, fileType) {
        const lines = content.split('\n');
        let nestingLevel = 0;
     ...
     */
    getLoopBody(lines, loopStartLine) {
     ...
     */
    displayBottleneckResults(bottlenecks) {
    ...
    generateBottleneckReport(bottlenecks) {
        ...
        vscode.window.showInformationMessage('Bottleneck analysis report generated', 'Open Report').then(selection => {
