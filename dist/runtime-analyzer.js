...
class RuntimeAnalyzer {
    ...
    startRecording() {
        ...
        this.recordingStartTime = perf_hooks_1.performance.now();
    ...
    markStart(markerId) {
        ...
        this.perfMarkers.set(markerId, perf_hooks_1.performance.now());
     ...
     */
    recordPerformanceTrendPoint(markerId, executionTime) {
        if (!this.performanceTrends.has(markerId)) {
            this.performanceTrends.set(markerId, []);
        }
        const relativeTimestamp = perf_hooks_1.performance.now() - this.recordingStartTime;
     ...
     */
    captureMemoryUsage(markerId, type) {
     ...
     */
    startAsyncOperation(markerId, operationId) {
        if (!this.isRecording) {
            return;
        }
     ...
     * @param operationId Unique identifier for this specific async operation
     */
    endAsyncOperation(markerId, operationId) {
        ...
        if (opIndex !== -1) {
            operations[opIndex].endTime = perf_hooks_1.performance.now();
    ...
    generateChartJs(data) {
        ...
        if (data.trendData) {
            ...
            new Chart(document.getElementById('trendChart${data.chartId}'), {
                type: 'line',
                data: {
                    labels: ${JSON.stringify(data.trendPoints.map((p, i) => 'Point ' + (i + 1)))},
                    datasets: [{
                        label: 'Execution Time Trend',
                        data: ${JSON.stringify(data.trendPoints.map((p) => p.executionTime))},
