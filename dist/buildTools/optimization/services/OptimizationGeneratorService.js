...
class OptimizationGeneratorService {
    ...
    async generateOptimizations(buildScripts, packageJson) {
        const optimizations = [];
    ...
    async applyOptimizations(packageJson, optimizations) {
        let modified = false;
    ...
    generateCrossEnvOptimizations(context) {
    ...
    generateParallelizationOptimizations(context) {
        const { scriptInfo, analysis } = context;
        const optimizations = [];
        if (scriptInfo.command.includes('&&') && !analysis.isParallel) {
            optimizations.push({
                ...
                complexity: 'medium',
                before: scriptInfo.command,
                after: `concurrently "${scriptInfo.command.split('&&').map((cmd) => cmd.trim()).join('" "')}"`,
                requiredPackages: ['concurrently']
            });
        }
        return optimizations;
    }
    generateWebpackOptimizations(context) {
        const { scriptInfo, analysis } = context;
        const optimizations = [];
    ...
    generateTypeScriptOptimizations(context) {
        const { scriptInfo, analysis } = context;
        const optimizations = [];
    ...
    generateCleanupOptimizations(context) {
        const { scriptInfo, analysis } = context;
        const optimizations = [];
    ...
    generateCacheOptimizations(context) {
        const { scriptInfo, analysis } = context;
        const optimizations = [];
    ...
    generateEnvironmentOptimizations(context) {
...
//# sourceMappingURL=OptimizationGeneratorService.js.map