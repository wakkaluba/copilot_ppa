...
class SnippetsPanelProvider {
    ...
    open() {
        ...
        this.disposables.push(this.snippetManager.onSnippetAdded(() => this.updateWebviewContent()), this.snippetManager.onSnippetUpdated(() => this.updateWebviewContent()), this.snippetManager.onSnippetDeleted(() => this.updateWebviewContent()));
    ...
    async handleCreateSnippet(title, content, tags) {
        try {
    ...
    async handleUpdateSnippet(id, updates) {
        try {
    ...
    async handleOpenSource(id) {
        const snippet = this.snippetManager.getSnippet(id);
        if (snippet && snippet.sourceConversationId) {
            await vscode.commands.executeCommand('copilotPPA.openConversation', snippet.sourceConversationId);
        }
        else {
            vscode.window.showErrorMessage('Source conversation not found');
        }
    }
    getWebviewContent(snippets) {
        // Get all unique tags
        ...
        <body>
            ...
            <div class="tag-pills">
                ...
                ${Array.from(allTags).map(tag => `<div class="tag-pill" data-tag="${tag}">${tag}</div>`).join('')}
            </div>
