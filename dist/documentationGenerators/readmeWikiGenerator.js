...
class ReadmeWikiGenerator {
        ...
        try {
            ...
            if (readmeContent) {
                ...
                if (fs.existsSync(readmePath)) {
                    const overwrite = await vscode.window.showWarningMessage('README.md already exists. Do you want to overwrite it?', { modal: true }, 'Yes', 'No', 'Compare');
                    if (overwrite === 'No') {
                        return;
                    }
        ...
        try {
            ...
            if (contributingContent) {
                ...
                if (fs.existsSync(contributingPath)) {
                    const overwrite = await vscode.window.showWarningMessage('CONTRIBUTING.md already exists. Do you want to overwrite it?', { modal: true }, 'Yes', 'No', 'Compare');
                    if (overwrite === 'No') {
                        return;
                    }
                    else if (overwrite === 'Compare') {
    ...
    async generateWikiPage() {
        try {
            ...
            const wikiPageType = await vscode.window.showQuickPick(Object.values(DocumentationType).filter(type => type.startsWith('Wiki')), { placeHolder: 'Select the type of Wiki page to generate' });
            if (!wikiPageType) {
            ...
            if (wikiContent) {
                ...
                if (fs.existsSync(wikiPath)) {
                    const overwrite = await vscode.window.showWarningMessage(`${filename} already exists. Do you want to overwrite it?`, { modal: true }, 'Yes', 'No', 'Compare');
                    if (overwrite === 'No') {
                        return;
                    }
                    else if (overwrite === 'Compare') {
            ...
            vscode.window.withProgress({
                ...
                for (const docType of docTypes) {
                    ...
                    if (docType.label === 'README.md') {
                        ...
                        const wikiType = Object.values(DocumentationType).find(type => type === docType.label) || DocumentationType.CUSTOM;
     ...
     */
    async generateSpecificWikiPage(wikiType) {
        try {
            const workspaceFolders = vscode.workspace.workspaceFolders;
            if (!workspaceFolders) {
                return;
    ...
    async gatherProjectInfo(projectRoot) {
        ...
        try {
            ...
            projectInfo.configFiles = configFiles.filter(file => fs.existsSync(path.join(projectRoot, file)));
...
The README should be comprehensive but clear and concise. Use proper Markdown formatting including headings, lists, code blocks, and tables where appropriate.`;
     ...
     * @returns Prompt for the LLM
     */
    createContributingPrompt(projectInfo) {
...
Use markdown formatting and create clear instructions for potential contributors. The guide should be friendly and welcoming while also setting clear expectations.`;
     ...
     * @returns Prompt for the LLM
     */
    createWikiPagePrompt(wikiPageType, projectInfo) {
...
Use proper Markdown formatting including headings, lists, code blocks, and tables where appropriate. Make the page readable, comprehensive, and user-friendly.`;
     ...
     */
    async showDiff(existingContent, newContent, title) {
