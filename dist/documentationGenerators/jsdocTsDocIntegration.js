...
class JSDocTSDocIntegration {
    ...
    registerCommands() {
        ...
        this.context.subscriptions.push(vscode.commands.registerCommand('localLLMAgent.generateDocumentation.selection', async () => await this.generateDocumentationForSelection()));
        // Command to generate documentation for current file
        this.context.subscriptions.push(vscode.commands.registerCommand('localLLMAgent.generateDocumentation.file', async () => await this.generateDocumentationForFile()));
        // Command to generate documentation for entire project
        this.context.subscriptions.push(vscode.commands.registerCommand('localLLMAgent.generateDocumentation.project', async () => await this.generateDocumentationForProject()));
    }
    /**
     * Generate documentation for the selected code
     */
    async generateDocumentationForSelection() {
        ...
        const prompt = this.createDocumentationPrompt(selectedText, documentationStyle, editor.document.languageId, includeExamples, includeTypes);
        try {
     ...
     */
    async generateDocumentationForFile() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor found');
            return;
        ...
        const includeExamples = this.shouldIncludeExamples();
        const includeTypes = this.shouldIncludeTypes();
        const prompt = this.createDocumentationPrompt(fileContent, documentationStyle, editor.document.languageId, includeExamples, includeTypes, true // isFullFile
        );
        try {
     ...
     */
    async generateDocumentationForProject() {
            ...
            try {
                ...
                for (const file of allFiles) {
                    ...
                    try {
                        ...
                        const prompt = this.createDocumentationPrompt(fileContent, documentationStyle, languageId, includeExamples, includeTypes, true);
                        const response = await this.llmProvider.sendPrompt(prompt);
                        if (response) {
                            ...
                            await vscode.workspace.fs.writeFile(vscode.Uri.file(docFilePath), Buffer.from(docContent, 'utf8'));
                            processedFiles++;
                            progress.report({
     ...
     */
    createDocumentationPrompt(code, style, languageId, includeExamples, includeTypes, isFullFile = false) {
...
`;
     ...
     */
    async generateDocumentationForCurrentNode() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor found');
            return;
        ...
        const includeExamples = this.shouldIncludeExamples();
        const includeTypes = this.shouldIncludeTypes();
        const prompt = this.createDocumentationPrompt(selectedText, documentationStyle, document.languageId, includeExamples, includeTypes);
        try {
            vscode.window.showInformationMessage('Generating documentation...');
            const response = await this.llmProvider.sendPrompt(prompt);
            if (response) {
