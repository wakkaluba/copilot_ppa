...
class TypeScriptAnalyzer extends baseAnalyzer_1.BasePerformanceAnalyzer {
    options;
    constructor(options) {
        super(options);
    ...
    calculateAverageMethodLength(content) {
        ...
        if (methods.length === 0)
            return 0;
        const totalLines = methods.reduce((sum, method) => sum + method.split('\n').length, 0);
        return Math.round(totalLines / methods.length);
    }
    analyzeTypeScriptAntipatterns(fileContent, lines) {
        const issues = [];
