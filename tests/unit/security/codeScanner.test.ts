...
import { CodeSecurityScanner } from '../../../src/security/codeScanner';
import { CodeScanResult, SecurityIssue, SecurityPattern } from '../../../src/security/types';
...
class MockSecurityAnalyzerService {
  constructor(private patternService: any) {}

  async scanDocument(document: vscode.TextDocument): Promise<{ issues: SecurityIssue[], diagnostics: vscode.Diagnostic[] }> {
  ...
  async scanWorkspace(progressCallback?: (message: string) => void): Promise<CodeScanResult> {
    if (progressCallback) {
...
class MockSecurityDiagnosticService {
  ...
  report(fileUri: vscode.Uri, diagnostics: vscode.Diagnostic[]): void {
    // Mock implementation
  }
...
class MockSecurityFixService {
  ...
  async applyFix(issueId: string, filePath: string): Promise<boolean> {
    return true;
  }
}
...
describe('Code Security Scanner Tests', () => {
  ...
  let mockPatternService: any;
  let mockAnalyzerService: any;
  let mockDiagnosticService: any;
  let mockFixService: any;
  let windowStub: sinon.SinonStub;
  let openTextDocumentStub: sinon.SinonStub;
  let editorStub: sinon.SinonStub;
  let webviewPanelStub: sinon.SinonStub;
  ...
  beforeEach(() => {
    ...
    const CodeSecurityScannerProxy = function(context: vscode.ExtensionContext) {
      const instance = new originalScanner(context);
      // Replace the dependencies with our mocks
      (instance as any).patternService = mockPatternService;
      (instance as any).analyzerService = mockAnalyzerService;
      (instance as any).diagnosticService = mockDiagnosticService;
      (instance as any).fixService = mockFixService;
      return instance;
    };
  ...
  test('Should scan individual file successfully', async () => {
    ...
    sinon.assert.calledOnce(mockAnalyzerService.scanDocument as sinon.SinonSpy);
    sinon.assert.calledOnce(mockDiagnosticService.report as sinon.SinonSpy);
  ...
  test('Should scan workspace successfully', async () => {
    ...
    sinon.assert.calledOnce(mockAnalyzerService.scanWorkspace as sinon.SinonSpy);
    sinon.assert.calledWith(mockAnalyzerService.scanWorkspace as sinon.SinonSpy, progressCallbackSpy);
  ...
  test('Should show security report with correct HTML', async () => {
    const mockResult: CodeScanResult = {
      issues: [
        {
          id: 'issue-1',
          patternId: 'sql-injection',
  ...
  test('Should register and handle webview messages', async () => {
    ...
    scanner.registerWebview('test-webview', mockWebview as unknown as vscode.Webview);
    ...
    sinon.assert.calledOnce(mockFixService.applyFix as sinon.SinonSpy);
    sinon.assert.calledWith(mockFixService.applyFix as sinon.SinonSpy, 'issue-1', '/mock/file.js');
  });
  ...
  test('Should properly unregister webview', async () => {
    ...
    (scanner as any).webviewMap.has('test-webview');

    scanner.unregisterWebview('test-webview');

    // Verify it was unregistered
    assert.strictEqual((scanner as any).webviewMap.has('test-webview'), false);
  });
  ...
  test('Should properly dispose of all resources', async () => {
    ...
    (scanner as any).messageQueue.push(async () => {});

    // Create disposable spy
    const disposableSpy = sandbox.spy((scanner as any).disposables[0], 'dispose');

    scanner.dispose();

    sinon.assert.calledOnce(mockDiagnosticService.dispose as sinon.SinonSpy);
    sinon.assert.calledOnce(disposableSpy);

    // Verify internal state cleanup
    assert.strictEqual((scanner as any).webviewMap.size, 0);
    assert.strictEqual((scanner as any).messageQueue.length, 0);
    assert.strictEqual((scanner as any).disposables.length, 0);
    assert.strictEqual((scanner as any).issueCache.size, 0);
  });
  ...
  test('Should properly process message queue in order', async () => {
    ...
    (scanner as any).messageQueue = [handler1, handler2, handler3];

    // Process the queue
    await (scanner as any).processMessageQueue();
    ...
    assert.strictEqual((scanner as any).isProcessing, false);
  });
  ...
  test('Should handle error in message queue processing', async () => {
    ...
    (scanner as any).messageQueue = [errorHandler, successHandler];
  ...
  test('Should not start processing queue if already processing', async () => {
    // Set processing flag
    (scanner as any).isProcessing = true;

    // Try to process queue
    await (scanner as any).processMessageQueue();

    // Verify it returned early
    assert.strictEqual((scanner as any).isProcessing, true);
  });
});
