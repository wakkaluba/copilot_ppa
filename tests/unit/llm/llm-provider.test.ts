...
class MockLLMProvider extends EventEmitter implements LLMProvider {
    ...
    private _mockResponses: Map<string, LLMResponse> = new Map();
    private _mockStreamingResponses: Map<string, string[]> = new Map();
    private _offlineMode: boolean = false;
    private _cachedResponses: Map<string, LLMResponse> = new Map();
    private _models: Map<string, {
        id: string;
        name: string;
        provider: string;
        parameters: number;
    ...
    async completePrompt(request: { prompt: string, options?: LLMRequestOptions }): Promise<LLMResponse> {
    ...
    async *streamPrompt(request: { prompt: string, options?: LLMRequestOptions }): AsyncIterable<LLMResponse> {
        if (!this._connected && !this._offlineMode) {
            throw new Error('Provider not connected');
        }
    ...
    async useCachedResponse(prompt: string): Promise<LLMResponse | null> {
    ...
    setMockStreamingResponse(prompt: string, chunks: string[]): void {
        this._mockStreamingResponses.set(prompt, chunks);
    }

    addModel(modelId: string, modelInfo: any): void {
    ...
    setStatus(status: 'active' | 'inactive' | 'error'): void {
        this._status = status;
...
describe('LLMProvider Interface', () => {
    ...
    describe('Text Completion', () => {
        ...
        test('should include system prompt when provided', async () => {
            ...
            const userPrompt = 'Tell me about TypeScript';
