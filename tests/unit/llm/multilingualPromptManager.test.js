...
const { MultilingualPromptManager } = require('../../../src/llm/multilingualPromptManager');

describe('MultilingualPromptManager (JavaScript)', () => {
  let promptManager;
  ...
  describe('getLanguageName behavior', () => {
    it('should correctly map language codes to names through public methods', () => {
      ...
      for (const [code, expectedName] of Object.entries(languageCodeToExpectedName)) {
        const result = promptManager.enhancePromptWithLanguage('test', code);
  ...
  describe('JavaScript-specific behavior', () => {
    it('should handle non-string prompts by converting them to strings', () => {
      // Test with a number
      const result1 = promptManager.enhancePromptWithLanguage(42, 'es');
      expect(result1).toBe('42\n\nRespond in Spanish.');

      // Test with an object
      const result2 = promptManager.enhancePromptWithLanguage({ test: 'object' }, 'fr');
      expect(result2).toContain('[object Object]');
      expect(result2).toContain('Respond in French');

      // Test with null
      const result3 = promptManager.enhancePromptWithLanguage(null, 'de');
      expect(result3).toContain('null');
      expect(result3).toContain('Respond in German');

      // Test with undefined
      const result4 = promptManager.enhancePromptWithLanguage(undefined, 'it');
      expect(result4).toContain('undefined');
      expect(result4).toContain('Respond in Italian');
    });

    it('should handle invalid language parameters', () => {
      // Test with null
      const result1 = promptManager.enhancePromptWithLanguage('test', null);
      expect(result1).toContain('Respond in English');

      // Test with undefined
      const result2 = promptManager.enhancePromptWithLanguage('test', undefined);
      expect(result2).toContain('Respond in English');

      // Test with a number
      const result3 = promptManager.enhancePromptWithLanguage('test', 42);
      expect(result3).toContain('Respond in English');

      // Test with an object
      const result4 = promptManager.enhancePromptWithLanguage('test', {});
      expect(result4).toContain('Respond in English');
    });

    it('should handle isResponseInExpectedLanguage with non-string input', () => {
      // Null response
      expect(promptManager.isResponseInExpectedLanguage(null, 'es')).toBe(true);

      // Undefined response
      expect(promptManager.isResponseInExpectedLanguage(undefined, 'fr')).toBe(true);

      // Number response (won't have the English-only phrases)
      expect(promptManager.isResponseInExpectedLanguage(42, 'de')).toBe(true);

      // Object response
      expect(promptManager.isResponseInExpectedLanguage({}, 'it')).toBe(true);
    });

    it('should handle buildLanguageCorrectionPrompt with unexpected types', () => {
      // Test with various types for all parameters
      const result = promptManager.buildLanguageCorrectionPrompt(
        123,
        [1, 2, 3],
        null
      ...
      expect(result).toContain('Original prompt: 123');
      expect(result).toContain('Response to translate: 1,2,3');
      expect(result).toContain('translated to English');
    });
  });
});
