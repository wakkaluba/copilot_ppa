// filepath: d:\___coding\tools\copilot_ppa\tests\unit\commands\debugCommands.test.js
const vscode = require('vscode');
const { describe, expect, test, beforeEach, jest, afterEach } = require('@jest/globals');
const { registerDebugCommands } = require('../../../src/commands/debugCommands');
const { CopilotCommunicationAnalyzer } = require('../../../src/debug/copilotCommunicationAnalyzer');
const { DebugDashboard } = require('../../../src/debug/debugDashboard');
const { DebugConfigPanel } = require('../../../src/debug/debugConfigPanel');
const { LogViewer } = require('../../../src/debug/logViewer');
const { CudaDetector } = require('../../../src/debug/cudaDetector');
const { ModelCompatibilityChecker } = require('../../../src/debug/modelCompatibilityChecker');
...
describe('Debug Commands JS', () => {
  ...
  beforeEach(() => {
    ...
    mockAnalyzer = CopilotCommunicationAnalyzer.getInstance();
    mockDashboard = DebugDashboard.getInstance();
    mockConfigPanel = DebugConfigPanel.getInstance();
    mockLogViewer = LogViewer.getInstance();
    mockCudaDetector = CudaDetector.getInstance();
    mockModelChecker = ModelCompatibilityChecker.getInstance();
    mockLogger = AdvancedLogger.getInstance();
  ...
  test('showDebugDashboard command should show the debug dashboard', () => {
    ...
    const showDebugDashboardHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('showDebugConfig command should show the debug configuration panel', () => {
    // Get the registered command handler
    const showDebugConfigHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('showLogViewer command should show the log viewer', () => {
    // Get the registered command handler
    const showLogViewerHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('toggleCommunicationAnalyzer command should toggle analyzer state', async () => {
    // Get the registered command handler
    const toggleCommunicationAnalyzerHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('exportCommunicationData command should export history to file', async () => {
    // Get the registered command handler
    const exportCommunicationDataHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('clearCommunicationHistory command should clear history', () => {
    // Get the registered command handler
    const clearCommunicationHistoryHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('checkCudaSupport command should detect CUDA and show results', async () => {
    // Get the registered command handler
    const checkCudaSupportHandler = vscode.commands.registerCommand.mock.calls.find(
      call => call[0] === 'copilot-ppa.checkCudaSupport'
    )[1];

    // Mock showInformationMessage to simulate clicking "Show Details"
    vscode.window.showInformationMessage.mockResolvedValue('Show Details');
  ...
  test('checkModelCompatibility command should show compatibility report', async () => {
    // Get the registered command handler
    const checkModelCompatibilityHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('getModelRecommendations command should get recommendations and show results', async () => {
    // Get the registered command handler
    const getModelRecommendationsHandler = vscode.commands.registerCommand.mock.calls.find(
      call => call[0] === 'copilot-ppa.getModelRecommendations'
    )[1];

    // Mock showInformationMessage to simulate clicking "Show Recommendations"
    vscode.window.showInformationMessage.mockResolvedValue('Show Recommendations');
  ...
  test('clearLogs command should clear logs', () => {
    // Get the registered command handler
    const clearLogsHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('showOutputChannel command should show output channel', () => {
    // Get the registered command handler
    const showOutputChannelHandler = vscode.commands.registerCommand.mock.calls.find(
  ...
  test('exportLogs command should show export options and export logs', async () => {
    // Get the registered command handler
    const exportLogsHandler = vscode.commands.registerCommand.mock.calls.find(
