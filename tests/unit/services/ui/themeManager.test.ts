...
describe('ThemeManager', () => {
    ...
    describe('Theme Management', () => {
        ...
        test('should delete custom theme', () => {
            // Create a custom theme and get its ID from the return value
            const newTheme = themeManager.createCustomTheme('Custom Theme 2', 'default', {});
            const beforeDelete = themeManager.getThemes();
            expect(beforeDelete.some(t => t.name === 'Custom Theme 2')).toBe(true);

            // Use the actual ID generated by ThemeManager
            themeManager.deleteCustomTheme(newTheme.id);
    ...
    describe('VS Code Theme Integration', () => {
        test('should handle VS Code theme changes', () => {
            // We need to directly invoke the handler since we can't fully mock the event system
            const instance = ThemeManager.getInstance(mockContext);
            
            // Spy on the syncWithVSCodeTheme method since that's what handleVSCodeThemeChange calls
            const spy = jest.spyOn(instance as any, 'syncWithVSCodeTheme');
            
            // Directly call the private method to simulate the event
            (instance as any).handleVSCodeThemeChange({ kind: vscode.ColorThemeKind.Dark });
            
            // Verify the handler worked by checking if syncWithVSCodeTheme was called
